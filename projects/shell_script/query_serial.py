...
# read the UART byte per byte until EOL or timeout
...
# ouputs a csv formatted file
def write_csv(res) -> None:
    with open(f"{"caches-off" if caches else "cache-con"}-{"sctlr.c-off" if sctlr else "sctlr.c-on"}-{time.time()}.csv", "w") as f:
